...
_REQUIRED_SHELL_IO_CALLS = ("db.getOrder", "emailService.send", "db.updateStatus")
...
def _is_candidate_pure_block(block: str) -> bool:
    """Return True if the block looks like a pure-core function: sync named/arrow fn, or explicitly marked with '// pure core'."""
    return any([
...
def _candidate_pure_blocks(text: str) -> list[str]:
    """Filter all code blocks down to those that appear to be pure-core candidates."""
...
def _new_function_names(text: str) -> set[str]:
    """Collect named/arrow function identifiers introduced by the refactor, excluding known shell functions."""
    named = {
        m.group(1)
...
def _io_leaks_in_pure_blocks(text: str) -> list[tuple[str, str]]:
    """Collect (token, first-line snippet) pairs for every I/O token found inside a candidate pure-core block."""
...
def _missing_discount_elements(text: str) -> list[str]:
    """Return tier names and discount percentages absent from the refactored output."""
...
def _block_has_kind_discriminator(block: str) -> bool:
    """Return True if the block uses a 'kind' field — the FCIS idiom for returning a decision as data."""
...
def _suspicious_saga_fn_names(text: str) -> list[str]:
    """Find function names suggesting FCIS was incorrectly applied to a saga (decide*, plan*Saga)."""
    return [
        m.group(0)
        for block in _code_blocks(text)
...
def assert_introduces_pure_function(run: ClaudeRun) -> None:
    """Fail if the refactor adds no new named or arrow functions alongside the shell."""
...
def assert_pure_core_no_io(run: ClaudeRun) -> None:
...
def assert_shell_preserves_io(run: ClaudeRun) -> None:
...
def assert_preserves_discount_rules(run: ClaudeRun) -> None:
    """Fail if discount tier names or percentages are missing from the refactor."""
    missing = _missing_discount_elements(run.assistant_text)
    assert not missing, f"discount rules missing from refactor: {missing}"


def assert_alert_decision_extracted(run: ClaudeRun) -> None:
    """Fail if the alert decision is not expressed as data via a kind discriminant."""
...
def assert_adds_retry_loop(run: ClaudeRun) -> None:
    ...
    assert any(re.search(p, run.assistant_text, re.IGNORECASE) for p in patterns), (
        "no retry loop, retry helper, or backoff logic introduced"
    )


def assert_preserves_compensation(run: ClaudeRun) -> None:
    """Fail if saga compensation calls (refund/release) are dropped from the output."""
    text = run.assistant_text
    assert "paymentApi.refund" in text, "compensation lost: paymentApi.refund missing"
    assert "fulfillmentApi.release" in text, (
        "compensation lost: fulfillmentApi.release missing"
    )


def assert_no_pure_core_extraction(run: ClaudeRun) -> None:
    """Fail if FCIS framing or a pure-decision function appears in a saga refactor."""
    text_lc = run.assistant_text.lower()
    assert "// pure core" not in text_lc, (
        "saga refactor introduced a '// pure core' marker; FCIS shouldn't apply here"
    )
    assert "functional core" not in text_lc, (
        "saga refactor uses 'functional core' framing; FCIS shouldn't apply here"
    )
...
def assert_skill_not_invoked(run: ClaudeRun) -> None:
    ...
    assert not run.skill_invoked, (
        "FCIS skill was invoked on the saga prompt; this is the When-NOT-to-use case"
    )
